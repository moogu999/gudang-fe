    ...
    >
      ...
      <div class="mb-4 flex flex-col gap-2 md:flex-row md:items-start md:gap-4">
        <label for="branchId" class="w-full text-sm font-semibold sm:text-base md:w-48">
          {{ t('apInvoiceConfigs.fields.branch') }}
        </label>
        <div class="flex w-full flex-auto flex-col gap-1">
          <InfiniteSelect
            ...
            :placeholder="t('apInvoiceConfigs.labels.selectBranch')"
            sort-by="name"
            sort-operator="asc"
          />
      ...
      <div class="mb-4 flex flex-col gap-2 md:flex-row md:items-start md:gap-4">
        <label for="approvalFlowId" class="w-full text-sm font-semibold sm:text-base md:w-48">
          {{ t('apInvoiceConfigs.fields.approvalFlow') }}
        </label>
        <div class="flex w-full flex-auto flex-col gap-1">
          <InfiniteSelect
            id="approvalFlowId"
            ...
            :placeholder="t('apInvoiceConfigs.labels.noApprovalRequired')"
            show-clear
            :disabled="mode === DialogMode.VIEW"
            class="w-full"
          />
          <small class="text-surface-500">{{
            t('apInvoiceConfigs.labels.approvalFlowHint')
          }}</small>
          <Message
...
import { BranchesService, ApInvoiceConfigService, ApprovalsService } from '@/services'
import { commonErrorToast, commonSuccessToast } from '@/services/toast'
import type { Base } from '@/types/api.type'
import type { ApInvoiceConfig, ApprovalFlow } from '@/types'
...
const props = defineProps({
  ...
  config: {
    type: Object as PropType<ApInvoiceConfig>,
    default: undefined,
  },
})
...
const toastGroup = 'apInvoiceConfigDialog'
...
async function fetchApprovalFlows(): Promise<Base<ApprovalFlow>> {
  const flows = await ApprovalsService.listFlows('ap_invoice')
    ...
    z.object({
      branchId:
        props.mode === DialogMode.ADD
          ? z.number({ message: t('apInvoiceConfigs.validation.branchRequired') })
          : z.number().optional(),
...
async function onFormSubmit(event: FormSubmitEvent) {
  ...
  try {
    await ApInvoiceConfigService.upsert(branchId, {
      approvalFlowId: event.states.approvalFlowId.value ?? null,
    })

    const message =
      props.mode === DialogMode.ADD
        ? t('apInvoiceConfigs.messages.created')
        : t('apInvoiceConfigs.messages.updated')
