<template>
  <div class="overflow-x-auto rounded border border-stone-200 font-mono text-sm">
    ...
    <div
      v-if="props.prev === null && props.curr !== null"
      class="bg-green-100 px-4 py-2 font-semibold text-green-800"
    >
      {{ t('auditTrails.detail.created') }}
    </div>
    <div
      v-if="props.curr === null && props.prev !== null"
      class="bg-red-100 px-4 py-2 font-semibold text-red-800"
    >
    ...
    <!-- Column headers -->
    <div
      class="grid grid-cols-[3rem_1fr_3rem_1fr] border-b border-stone-200 bg-stone-100 text-xs text-stone-500"
    >
      <div class="border-r border-stone-200 px-2 py-1 text-center">#</div>
      <div class="border-r border-stone-200 px-3 py-1">{{ t('auditTrails.detail.previous') }}</div>
      <div class="border-r border-stone-200 px-2 py-1 text-center">#</div>
    ...
    >
      <!-- Left line number -->
      <div
        class="border-r border-stone-200 px-2 py-0.5 text-right text-xs text-stone-400 select-none"
        :class="row.leftClass"
      >
        {{ row.leftLineNum ?? '' }}
      </div>
      <!-- Left content -->
      <div
        class="min-w-0 border-r border-stone-200 px-3 py-0.5 whitespace-pre"
        :class="row.leftClass"
      >
        {{ row.left }}
      </div>
      ...
      <!-- Right content -->
      <div class="min-w-0 px-3 py-0.5 whitespace-pre" :class="row.rightClass">{{ row.right }}</div>
    </div>

    <div v-if="diffRows.length === 0" class="px-4 py-6 text-center text-sm text-stone-400">
  ...
  while (i < parts.length) {
    ...
    if (!part.added && !part.removed) {
      ...
      const removedLines = part.value
        .split('\n')
        .filter((_, idx, arr) => idx < arr.length - 1 || arr[idx] !== '')
      const addedLines = parts[i + 1].value
        .split('\n')
        .filter((_, idx, arr) => idx < arr.length - 1 || arr[idx] !== '')
