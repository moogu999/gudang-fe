    ...
    >
      ...
      <div class="mb-4 flex flex-col gap-2 md:flex-row md:items-start md:gap-4">
        <label for="branchId" class="w-full text-sm font-semibold sm:text-base md:w-48">
          {{ t('creditDebitNoteConfigs.fields.branch') }}
        </label>
        <div class="flex w-full flex-auto flex-col gap-1">
          <InfiniteSelect
            ...
            :placeholder="t('creditDebitNoteConfigs.labels.selectBranch')"
            sort-by="name"
            sort-operator="asc"
          />
      ...
      <div class="mb-4 flex flex-col gap-2 md:flex-row md:items-start md:gap-4">
        <label for="approvalFlowId" class="w-full text-sm font-semibold sm:text-base md:w-48">
          {{ t('creditDebitNoteConfigs.fields.approvalFlow') }}
        </label>
        <div class="flex w-full flex-auto flex-col gap-1">
          <InfiniteSelect
            id="approvalFlowId"
            ...
            :placeholder="t('creditDebitNoteConfigs.labels.noApprovalRequired')"
            show-clear
            :disabled="mode === DialogMode.VIEW"
            class="w-full"
          />
          <small class="text-surface-500">{{
            t('creditDebitNoteConfigs.labels.approvalFlowHint')
          }}</small>
          <Message
...
import { BranchesService, CreditDebitNoteConfigService, ApprovalsService } from '@/services'
import { commonErrorToast, commonSuccessToast } from '@/services/toast'
import type { Base } from '@/types/api.type'
import type { CreditDebitNoteConfig, ApprovalFlow } from '@/types'
...
const props = defineProps({
  ...
  config: {
    type: Object as PropType<CreditDebitNoteConfig>,
    default: undefined,
  },
})
...
const toastGroup = 'creditDebitNoteConfigDialog'
...
async function fetchApprovalFlows(): Promise<Base<ApprovalFlow>> {
  const flows = await ApprovalsService.listFlows('credit_debit_note')
    ...
    z.object({
      branchId:
        props.mode === DialogMode.ADD
          ? z.number({ message: t('creditDebitNoteConfigs.validation.branchRequired') })
          : z.number().optional(),
...
async function onFormSubmit(event: FormSubmitEvent) {
  ...
  try {
    await CreditDebitNoteConfigService.upsert(branchId, {
      approvalFlowId: event.states.approvalFlowId.value ?? null,
    })

    const message =
      props.mode === DialogMode.ADD
        ? t('creditDebitNoteConfigs.messages.created')
        : t('creditDebitNoteConfigs.messages.updated')
