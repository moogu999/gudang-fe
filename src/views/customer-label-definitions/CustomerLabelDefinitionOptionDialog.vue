    ...
    >
      <div class="mb-4 flex flex-col gap-2 md:flex-row md:items-start md:gap-4">
        <label for="value" class="w-full text-sm font-semibold sm:text-base md:w-32">{{
          t('customerLabelDefinitions.options.fields.value')
        }}</label>
...
import { CustomerLabelOptionsService } from '@/services'
import { commonErrorToast, commonSuccessToast } from '@/services/toast'
import { useAuthStore } from '@/stores'
import DialogMode from '@/constants/dialogMode'
import type { CustomerLabelOption } from '@/types'
...
const props = defineProps({
  ...
  option: {
    type: Object as PropType<CustomerLabelOption>,
  },
  definitionId: {
    type: Number,
    required: true,
...
const toastGroup = 'CustomerLabelOptionDialog'
...
const resolver = computed(() =>
  zodResolver(
    z.object({
      value: z.string().min(1, t('customerLabelDefinitions.options.validation.valueRequired')),
    }),
  ),
)
...
async function addOption(event: FormSubmitEvent) {
  await CustomerLabelOptionsService.create({
    customerLabelDefinitionId: props.definitionId,
    value: event.states.value.value,
    createdBy: authStore.userId!,
  })

  toast.add(
    commonSuccessToast(t('customerLabelDefinitions.options.messages.optionCreated'), toastGroup),
  )
}

async function editOption(event: FormSubmitEvent) {
  await CustomerLabelOptionsService.update(props.option!.id, {
    value: event.states.value.value,
    updatedBy: authStore.userId!,
  })

  toast.add(
    commonSuccessToast(t('customerLabelDefinitions.options.messages.optionUpdated'), toastGroup),
  )
}
</script>
