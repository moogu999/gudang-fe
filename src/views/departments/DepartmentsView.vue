<template>
  <div>
    ...
    <h1 class="mb-5 text-lg font-semibold md:text-2xl">{{ t('departments.title') }}</h1>

    <Toolbar v-if="canWrite" class="mb-5">
      <template #end>
        <Button :label="t('common.actions.add')" icon="pi pi-plus" @click="addDepartment"></Button>
      </template>
    </Toolbar>
...
const dialogHeader = computed(() => {
  if (dialogMode.value === DialogMode.ADD) {
    ...
    return t('departments.editDepartment')
  } else {
    return t('departments.viewDepartment')
  }
})
