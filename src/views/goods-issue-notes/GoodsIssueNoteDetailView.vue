<template>
  <div class="p-2 sm:p-4 lg:p-10">
    ...
    <template v-else-if="detail">
      <!-- Success banner -->
      <Message severity="success" class="mb-4" :closable="false">
        {{ t('goodsIssueNotes.banner') }}
      </Message>
          ...
          <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            <div class="flex flex-col gap-1">
              <span class="text-xs font-semibold text-stone-500 uppercase">{{
                t('goodsIssueNotes.fields.no')
              }}</span>
              <span class="font-medium">{{ detail.no }}</span>
            </div>
            <div class="flex flex-col gap-1">
              <span class="text-xs font-semibold text-stone-500 uppercase">{{
                t('goodsIssueNotes.fields.status')
              }}</span>
              <Tag
                class="w-fit"
                :severity="detail.status === 'issued' ? 'success' : 'danger'"
                :value="t(`goodsIssueNotes.status.${detail.status}`)"
              />
            </div>
            <div class="flex flex-col gap-1">
              <span class="text-xs font-semibold text-stone-500 uppercase">{{
              ...
              <span>{{ dayjs(detail.issueDate).format(DateFormat.DATE) }}</span>
            </div>
            <div class="flex flex-col gap-1">
              <span class="text-xs font-semibold text-stone-500 uppercase">{{
                t('goodsIssueNotes.fields.createdAt')
              }}</span>
              <span>{{ dayjs(detail.createdAt).format(DateFormat.DATE_TIME) }}</span>
            </div>
            <div class="flex flex-col gap-1">
              <span class="text-xs font-semibold text-stone-500 uppercase">{{
                t('goodsIssueNotes.fields.pickingListNo')
              }}</span>
              <RouterLink
                :to="{ name: 'PickingListDetail', params: { id: detail.pickingListId } }"
                class="text-primary font-medium hover:underline"
                >{{ detail.pickingListNo }}</RouterLink
              >
            </div>
            <div class="flex flex-col gap-1">
              <span class="text-xs font-semibold text-stone-500 uppercase">{{
                t('goodsIssueNotes.fields.deliveryNoteNo')
              }}</span>
              <RouterLink
    ...
    <Message v-else-if="!isLoading" severity="error">
      {{ t('goodsIssueNotes.messages.notFound') }}
    </Message>
  </div>
</template>
...
import type { GoodsIssueNoteDetail, GoodsIssueNoteItem } from '@/types'
...
const toastGroup = 'goodsIssueNoteDetail'
const detail = ref<GoodsIssueNoteDetail | null>(null)
...
const groupedSections = computed<Section[]>(() => {
  ...
  const sectionMap = new Map<string, GoodsIssueNoteItem[]>()
  ...
  const uncategorized: GoodsIssueNoteItem[] = []
...
function formatTierQty(item: GoodsIssueNoteItem): string {
...
function uomLabel(item: GoodsIssueNoteItem): string | undefined {
  const levels = pinnedToLevels(item.pinnedUom)
...
async function fetchDetail(id: number) {
  ...
  try {
    detail.value = await GoodsIssueNotesService.get(id)
  } catch (e) {
