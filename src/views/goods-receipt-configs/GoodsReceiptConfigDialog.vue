    ...
    >
      ...
      <div class="mb-4 flex flex-col gap-2 md:flex-row md:items-start md:gap-4">
        <label for="branchId" class="w-full text-sm font-semibold sm:text-base md:w-48">
          {{ t('goodsReceiptConfigs.fields.branch') }}
        </label>
        <div class="flex w-full flex-auto flex-col gap-1">
          <InfiniteSelect
            ...
            :placeholder="t('goodsReceiptConfigs.labels.selectBranch')"
            sort-by="name"
            sort-operator="asc"
          />
      ...
      <div class="mb-4 flex flex-col gap-2 md:flex-row md:items-start md:gap-4">
        <label for="approvalFlowId" class="w-full text-sm font-semibold sm:text-base md:w-48">
          {{ t('goodsReceiptConfigs.fields.approvalFlow') }}
        </label>
        <div class="flex w-full flex-auto flex-col gap-1">
          <InfiniteSelect
            id="approvalFlowId"
            ...
            :placeholder="t('goodsReceiptConfigs.labels.noApprovalRequired')"
            show-clear
            :disabled="mode === DialogMode.VIEW"
            class="w-full"
          />
          <small class="text-surface-500">{{
            t('goodsReceiptConfigs.labels.approvalFlowHint')
          }}</small>
          <Message
...
import { BranchesService, GoodsReceiptConfigService, ApprovalsService } from '@/services'
import { commonErrorToast, commonSuccessToast } from '@/services/toast'
import type { Base } from '@/types/api.type'
import type { GoodsReceiptConfig, ApprovalFlow } from '@/types'
...
const props = defineProps({
  ...
  config: {
    type: Object as PropType<GoodsReceiptConfig>,
    default: undefined,
  },
})
...
const toastGroup = 'goodsReceiptConfigDialog'
...
async function fetchApprovalFlows(): Promise<Base<ApprovalFlow>> {
  const flows = await ApprovalsService.listFlows('goods_receipt')
    ...
    z.object({
      branchId:
        props.mode === DialogMode.ADD
          ? z.number({ message: t('goodsReceiptConfigs.validation.branchRequired') })
          : z.number().optional(),
...
async function onFormSubmit(event: FormSubmitEvent) {
  ...
  try {
    await GoodsReceiptConfigService.upsert(branchId, {
      approvalFlowId: event.states.approvalFlowId.value ?? null,
    })

    const message =
      props.mode === DialogMode.ADD
        ? t('goodsReceiptConfigs.messages.created')
        : t('goodsReceiptConfigs.messages.updated')
