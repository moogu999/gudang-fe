    ...
    >
      <div class="mb-4 flex flex-col gap-2 md:flex-row md:items-start md:gap-4">
        <label for="name" class="w-full text-sm font-semibold sm:text-base md:w-32">{{
          t('productLabelDefinitions.fields.name')
        }}</label>
      ...
      <div v-if="mode === DialogMode.EDIT && props.definition" class="mb-4">
        ...
        <div class="mb-3 flex items-center justify-between">
          <h3 class="text-sm font-semibold sm:text-base md:text-lg">
            {{ t('productLabelDefinitions.options.title') }}
          </h3>
          <Button
            :label="t('productLabelDefinitions.options.addOption')"
            icon="pi pi-plus"
            size="small"
            @click="addOption"
          />
        ...
        <DataTable
          ...
          class="text-sm"
        >
          <Column field="value" :header="t('productLabelDefinitions.options.fields.value')" />
...
import type { ProductLabelDefinition, ProductLabelOption } from '@/types'
import ProductLabelOptionDialog from './ProductLabelOptionDialog.vue'
...
const props = defineProps({
  ...
  definition: {
    type: Object as PropType<ProductLabelDefinition>,
  },
})
...
const toastGroup = 'productLabelDefinitionDialog'
...
const resolver = computed(() =>
  zodResolver(
    z.object({
      name: z.string().min(1, t('productLabelDefinitions.validation.nameRequired')),
    }),
  ),
)
...
async function addDefinition(event: FormSubmitEvent) {
  ...
  await ProductLabelDefinitionsService.create({
    name: event.states.name.value,
    companyId: 1,
    createdBy: authStore.userId!,
  })

  toast.add(commonSuccessToast(t('productLabelDefinitions.messages.definitionCreated'), toastGroup))
}

async function editDefinition(event: FormSubmitEvent) {
  await ProductLabelDefinitionsService.update(props.definition!.id, {
    name: event.states.name.value,
    updatedBy: authStore.userId!,
  })

  toast.add(commonSuccessToast(t('productLabelDefinitions.messages.definitionUpdated'), toastGroup))
}

// Options management
const options = ref<ProductLabelOption[]>([])
...
async function loadOptions() {
  ...
  const query = new GenericQueryBuilder()
    .withFilter('product_label_definition_id', FilterOperator.EQUAL, props.definition.id)
    .build()

  isLoadingOptions.value = true
  try {
    const response = await ProductLabelOptionsService.list(query)
...
const selectedOption = ref<ProductLabelOption | undefined>(undefined)

const optionDialogHeader = computed(() => {
  if (optionDialogMode.value === DialogMode.ADD) {
    return t('productLabelDefinitions.options.addOption')
  }
  return t('productLabelDefinitions.options.editOption')
})
...
function editOption(option: ProductLabelOption) {
...
function onDeleteOptionClick(id: number) {
  confirmDelete(() => ProductLabelOptionsService.delete(id))
}
</script>
