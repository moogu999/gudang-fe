<template>
  <div>
    ...
    <div v-else-if="definitions.length === 0" class="py-4 text-center text-sm text-gray-500">
      {{ t('products.labels.noDefinitions') }}
    </div>
...
import type { ProductLabelDefinition, ProductLabelOption, ProductLabelValue } from '@/types'

const { t } = useI18n()
const toast = useToast()
const toastGroup = 'productSetLabelsDialog'
...
const props = defineProps({
  ...
  currentLabels: {
    type: Array as PropType<ProductLabelValue[]>,
    default: () => [],
  },
})
...
const definitions = ref<ProductLabelDefinition[]>([])
const optionsByDefinition = reactive<Record<number, ProductLabelOption[]>>({})
...
onBeforeMount(async () => {
  ...
  // Pre-populate from current labels
    ...
    await Promise.all(
      definitions.value.map(async (def) => {
        const query = new GenericQueryBuilder()
          .withFilter('product_label_definition_id', FilterOperator.EQUAL, def.id)
          .build()
        const optResponse = await ProductLabelOptionsService.list(query)
...
async function onSave() {
  ...
  try {
    ...
    await ProductsService.setLabels(props.productId, labels)
    toast.add(commonSuccessToast(t('products.labels.labelsUpdated'), toastGroup))
    emits('close')
  } catch (e) {
    toast.add(commonErrorToast(e, toastGroup))
  } finally {
